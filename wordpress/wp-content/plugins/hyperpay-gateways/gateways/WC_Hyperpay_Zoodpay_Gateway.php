...
class WC_Hyperpay_Zoodpay_Gateway extends Hyperpay_main_class
{
  /**
   ...
   */
  public $method_title = "Hyperpay Zoodpay Gateway";
  ...
  public function __construct()
  {
    ...
    // call zoodpay API to set payment configrations
    $this->configration();
   ...
   * based on Zoodpay API configration 
   * @param string $icon
   * @param string $id
   * 
   * @return string $icon
  ...
  public function set_icons($icon, $id): string
  {
    if ($id == $this->id) {
      ...
      wp_enqueue_script('hyperpay_script_modal',  HYPERPAY_PLUGIN_DIR . '/assets/js/modal.js', ['jquery'], false, true);


      $img = HYPERPAY_PLUGIN_DIR . "/assets/images/ZOODPAY-logo.png";
